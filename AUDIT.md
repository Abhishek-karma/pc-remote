...
| **SEC-08** | Android Client | First-pairing TOFU cert pinned before pairing authentication succeeds (certificate poisoning). | **P0** | `android-app/network/RemoteConnection.kt:318` | Only commit certificate pin to `PinStore` after successful `auth_ok` response from host. |
| **SEC-09** | Windows Agent | Check-then-add race condition in connection limits allowing concurrency bypass. | **P1** | `windows-agent/Program.cs:165` | Atomically acquire connection slot inside synchronized block before socket accept and release in `finally`. |
| **SEC-10** | Windows Agent | RFC 6455 reserved bits (RSV1-3), control frame length (>125), and FIN=false control frames not rejected. | **P1** | `windows-agent/WebSocketConnection.cs:130` | Strictly enforce RFC 6455 §5.2 and §5.5 frame constraints; reject non-zero RSV and invalid control frames. |
